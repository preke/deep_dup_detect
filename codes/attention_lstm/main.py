...
DA_lstm = model.DA_lstm(
        vocab_size      = args.embed_num,
        device          = args.device,
        word_matrix     = args.pretrained_weight,
...
if args.cuda:
        ...
        DA_lstm = DA_lstm.cuda()

if args.snapshot is not None:
    print('\nLoading model from {}...'.format(args.snapshot))
    DA_lstm.load_state_dict(torch.load(args.snapshot))
    if args.cuda:
        torch.cuda.set_device(args.device)
        DA_lstm = DA_lstm.cuda()
    ...
    try:
        train(train_iter=train_iter, vali_iter=vali_iter, model=DA_lstm, args=args)
    except KeyboardInterrupt:
        print(traceback.print_exc())
        print('\n' + '-' * 89)
        print('Exiting from training early')

test(test_iter=test_iter, model=DA_lstm, args=args)
